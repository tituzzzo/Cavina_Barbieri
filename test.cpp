...
TEST_CASE("Testing flock.hpp functions")
{
  ...
  {
    double a_factor = 1.;
    CHECK(fl::Vector2D{0., 0.} == alignment_rule(bird0, 2.5, a_factor, flock_test));
    CHECK(fl::Vector2D{-1., -1.} == alignment_rule(bird0, 3., a_factor, flock_test));
  ...
  {
    double c_factor = 1.;
    CHECK(fl::Vector2D{2., 0.} == cohesion_rule(bird0, 2.5, c_factor, flock_test));
    CHECK(fl::Vector2D{1., 1.5} == cohesion_rule(bird0, 3., c_factor, flock_test));
  ...
  {
    fl::Statistics average_velocity_stat{calc_average_velocity_norm(flock_test)};
  ...
  {
    fl::Statistics average_distance_stat{calc_average_bird_to_bird_distance(flock_test)};
