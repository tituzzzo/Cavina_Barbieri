...
struct FParametres
{
  ...
  // clang-format on
  {
    ...
    if (n_birds < 3 || n_birds > 1000) {
      throw std::runtime_error{
          "Flock parameter: wrong n_birds initialization."};
    }
    if (box_size < 50 || box_size > 200) {
      throw std::runtime_error{
          "Flock parameter: wrong box_size initialization."};
    }
    if (d_ < max_d_s) {
      throw std::runtime_error{"Flock parameter: wrong d initialization."};
    }
    if (d_s_ < 0
        || d_s_ > sqrt(pow(box_size, 2) / static_cast<float>(n_birds))) {
      throw std::runtime_error{"Flock parameter: wrong d_s initialization."};
    }
    if (s < 0 || s > 1) {
      throw std::runtime_error{"Flock parameter: wrong s initialization."};
    }
    if (a < 0 || a > 1) {
      throw std::runtime_error{"Flock parameter: wrong a initialization."};
    }
    if (c < 0 || c > 1) {
      throw std::runtime_error{"Flock parameter: wrong c initialization."};
    }
    if (max_bird_velocity < 10 || max_bird_velocity > 500) {
      throw std::runtime_error{
          "Flock parameter: wrong max_bird_velocity initialization."};
    }
  }
};
...
class Flock
{
 private:
  ...
  // this method applies all flock rules to a bird and prevents it from
  // exceeding the max_bird_velocity parameter
...
void limit_bird_velocity_to_value(Vector2D& velocity,
                                  const double max_bird_velocity_norm);
